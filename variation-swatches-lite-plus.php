<?php
	 ...
	 * Text Domain: variation-swatches-lite-plus
...
if ( ! defined( 'WOO_VARIATION_SWATCHES_PLUGIN_VERSION' ) ) {
	define( 'WOO_VARIATION_SWATCHES_PLUGIN_VERSION', '1.0.19' );
}
...
if ( ! defined( 'WVS_LITE_PLUS_ALL_FEATURES' ) ) {
	define( 'WVS_LITE_PLUS_ALL_FEATURES', true );
}

if ( ! defined( 'WOO_VARIATION_SWATCHES_MAYBE_PRO_PLUGIN_FILE' ) ) {
	$woo_variation_swatches_maybe_pro_plugin_file = sprintf('%s/variation-swatches-lite-plus-pro/variation-swatches-lite-plus-pro.php', wp_normalize_path( WP_PLUGIN_DIR ));
...
function woo_variation_swatches_missing_wc_notice() {
	...
	if ( ! class_exists( 'WooCommerce' ) ) {
		...
		printf(
			...
			'notice notice-error',
			wp_kses( __( '<strong>Variation Swatches for WooCommerce</strong> is an add-on of ', 'variation-swatches-lite-plus' ), array( 'strong' => array() ) ),
			esc_url( add_query_arg( $args, admin_url( 'plugin-install.php' ) ) ),
			esc_html__( 'WooCommerce', 'variation-swatches-lite-plus' )
		);
	}
}
...
function woo_variation_swatches_deactivate_pro() {
	...
	if ( is_plugin_active( 'variation-swatches-lite-plus-pro/variation-swatches-lite-plus-pro.php' ) ) {
		...
		deactivate_plugins( 'variation-swatches-lite-plus-pro/variation-swatches-lite-plus-pro.php', true );
	}
}
...
function woo_variation_swatches_deactivate_notice_pro() {
	...
	$notice_text =  sprintf(esc_html__('You are running older version of "Variation Swatches for WooCommerce - Pro". Please upgrade to %s or upper and continue.', 'variation-swatches-lite-plus'), esc_html(constant( 'WOO_VARIATION_SWATCHES_MINIMUM_COMPATIBLE_PRO_PLUGIN_VERSION' )));
...
function woo_variation_swatches_row_meta_notice_pro( string $plugin_file, array $plugin_data) {
	if ( plugin_basename( WOO_VARIATION_SWATCHES_MAYBE_PRO_PLUGIN_FILE ) === $plugin_file ) {
		...
		if (  version_compare( $current_version, constant( 'WOO_VARIATION_SWATCHES_MINIMUM_COMPATIBLE_PRO_PLUGIN_VERSION' ), '<' )  ) {
			/* translators: %s: Pro Plugin Version */
			$notice_text = 	 sprintf(esc_html__('You are running older version of "Variation Swatches for WooCommerce - Pro". Please upgrade to %s or upper.', 'variation-swatches-lite-plus'), esc_html(constant( 'WOO_VARIATION_SWATCHES_MINIMUM_COMPATIBLE_PRO_PLUGIN_VERSION' )));
