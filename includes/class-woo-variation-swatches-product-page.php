	...
	class Woo_Variation_Swatches_Product_Page {
		...
		public function add_variation_data( $variation_data, $product, $variation ) {
			...
			if ( woo_variation_swatches()->is_pro() && wc_string_to_bool( woo_variation_swatches()->get_option( 'show_variation_stock_info', 'no' ) ) ) {
				...
				$variation_data[ 'variation_stock_left' ] = $variation->managing_stock() ? sprintf( esc_html__( '%s left', 'variation-swatches-lite-plus' ), $variation->get_stock_quantity() ) : '';
			}
		...
		public function enqueue_scripts() {
			...
			if ( wc_string_to_bool( woo_variation_swatches()->get_option( 'enable_stylesheet', 'yes' ) ) ) {
				wp_enqueue_style( 'variation-swatches-lite-plus', woo_variation_swatches()->assets_url( "/css/frontend{$suffix}.css" ), array(), woo_variation_swatches()->assets_version( "/css/frontend{$suffix}.css" ) );
			}
			...
			wp_register_script( 'variation-swatches-lite-plus', woo_variation_swatches()->assets_url( "/js/frontend{$suffix}.js" ), array(
				'jquery',
				'wp-util',
				'underscore',
			...
			if ( $extra_params_for_rest_uri ) {
				...
				wp_add_inline_script( 'variation-swatches-lite-plus', sprintf( 'wp.apiFetch.use( window.createMiddlewareForExtraQueryParams(%s) )', wp_json_encode( $extra_params_for_rest_uri ) ) );
			}

			wp_localize_script( 'variation-swatches-lite-plus', 'woo_variation_swatches_options', $this->js_options() );

			// @TODO: we need to load swatches script based on 'wc-add-to-cart-variation' script
			wp_enqueue_script( 'variation-swatches-lite-plus' );
		}
		...
		public function add_inline_style() {
			...
			wp_add_inline_style( 'variation-swatches-lite-plus', $style );
		}
		...
		public function dropdown( $html, $args ) {
			...
			$args = wp_parse_args( apply_filters( 'woocommerce_dropdown_variation_attribute_options_args', $args ), array(
				...
				'id'               => '',
				'class'            => '',
				'show_option_none' => esc_html__( 'Choose an option', 'variation-swatches-lite-plus' ),
				'is_archive'       => false
			) );
			...
			$show_option_none_text = $args[ 'show_option_none' ] ? $args[ 'show_option_none' ] : esc_html__( 'Choose an option', 'variation-swatches-lite-plus' ); // We'll do our best to hide the placeholder, but we'll need to show something when resetting options.
