...
if ( ! class_exists( 'Woo_Variation_Swatches_Frontend' ) ) {
	class Woo_Variation_Swatches_Frontend {
		...
		public function archive_swatches_output() {
			...
			if ( ! wc_string_to_bool( woo_variation_swatches()->get_option( 'show_on_archive', 'yes' ) ) ) {
				return;
			}

			$position = woo_variation_swatches()->get_option( 'archive_swatches_position', 'after' );
			$current  = current_filter();

			if ( 'before' === $position && 'woocommerce_after_shop_loop_item_title' !== $current ) {
				return;
			}

			if ( 'after' === $position && 'woocommerce_after_shop_loop_item' !== $current ) {
				return;
			}
