...
if ( ! class_exists( 'Woo_Variation_Swatches_Export_Import' ) ) :
	...
	class Woo_Variation_Swatches_Export_Import {
		...
		public function default_import_column_name( $columns ) {
			...
			$columns[ esc_html__( 'Swatches Attributes', 'variation-swatches-lite-plus' ) ] = $this->column_id;
		...
		public function get_attachment_id_from_url( $url, $product_id ) {
			...
			if ( ! $id && stristr( $url, '://' ) ) {
				...
				if ( ! wp_attachment_is_image( $id ) ) {
					...
					throw new Exception( sprintf( __( 'Not able to attach "%s".', 'variation-swatches-lite-plus' ), $url ), 400 );
				}
			...
			if ( ! $id ) {
				/* translators: %s: image URL */
				throw new Exception( sprintf( __( 'Unable to use image "%s".', 'variation-swatches-lite-plus' ), $url ), 400 );
			}
